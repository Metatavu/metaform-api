...
class ReplyDAO : AbstractDAO<Reply>() {
  ...
  private fun getFieldFilterSubqueryReplyField(
    ...
    return when (storeDataType) {
      StoreDataType.STRING -> return (fieldRoot as Root<StringReplyField>).get(StringReplyField_.value)
      StoreDataType.BOOLEAN -> return (fieldRoot as Root<BooleanReplyField>).get(BooleanReplyField_.value)
      StoreDataType.NUMBER -> return (fieldRoot as Root<NumberReplyField>).get(NumberReplyField_.value)
      else -> {
